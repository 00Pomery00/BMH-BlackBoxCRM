...
export default function CompanyDetail({ company, onClose, onDelete }) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50" role="presentation" onClick={onClose}>
      <div className="bg-white rounded-lg w-11/12 sm:w-2/3 md:w-1/2 p-6 shadow-lg" role="dialog" aria-modal="true" aria-labelledby="company-title" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-start">
          ...
          <button ref={closeRef} onClick={onClose} className="text-gray-500 hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-300 rounded" aria-label={t('close')}>✕</button>
        </div>
        <div className="mt-4">
          ...
          {edit ? (
            ...
            <p className="text-sm text-gray-700 mt-2">{company.description || t('no_description')}</p>
          )}
        </div>
        <div className="mt-6 flex gap-2 flex-wrap">
          ...
          {edit && (
            <>
              ...
              <button onClick={() => { setEdit(false); setForm(company); }} className="px-4 py-2 border rounded">
                {t('cancel')}
              </button>
            </>
          )}
          {confirmDelete && (
            <>
              ...
              <button onClick={() => { onDelete && onDelete(company.id); setConfirmDelete(false); }} className="px-4 py-2 bg-red-600 text-white rounded">{t('delete') || 'Smazat'}</button>
              <button onClick={() => setConfirmDelete(false)} className="px-4 py-2 border rounded">{t('cancel')}</button>
            </>
          )}
          <button onClick={onClose} className="px-4 py-2 border rounded">{t('close')}</button>
        </div>
      </div>
    </div>
  );
