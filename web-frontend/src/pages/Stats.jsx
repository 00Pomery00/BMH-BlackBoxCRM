...
export default function Stats() {
  ...
  return (
    <section
      ...
      data-id="stats-section"
      data-category="panel"
    >
      <h2 className="text-xl font-semibold mb-4" data-id="stats-title" data-category="nadpis">
        {t('statistics')}
      </h2>
      <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-gray-50 rounded p-4 shadow-sm">
          ...
          <SimpleBar
            ...
            label={t('won_leads', 'Vyhrané leady')}
            color="bg-green-500"
          />
          <SimpleBar
            value={stats.lostLeads}
            max={stats.totalLeads}
            label={t('lost_leads', 'Ztracené leady')}
            color="bg-red-400"
          />
        </div>
        <div className="bg-gray-50 rounded p-4 shadow-sm">
          ...
          <div className="mb-2 text-2xl font-bold">
            {stats.commission.toLocaleString('cs-CZ')} Kč
          </div>
          <SimpleBar
            value={stats.commission}
            max={stats.commissionGoal}
            label={t('commission_goal', 'Cíl provizí')}
            color="bg-violet-600"
          />
        </div>
      </div>
