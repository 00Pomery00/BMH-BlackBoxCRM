"""migrate users data into fu_users
...
def upgrade():
    ...
    # Attempt to copy users -> fu_users in an idempotent way.
    try:
        res = conn.execute(sa.text("SELECT id, email, hashed_password, is_active FROM users WHERE email IS NOT NULL"))
        rows = res.fetchall()
        for r in rows:
            email = r['email']
            hashed = r['hashed_password'] or ''
            is_active = bool(r['is_active'])
            # Use INSERT OR IGNORE for SQLite, INSERT ... ON CONFLICT DO NOTHING for others
            try:
                conn.execute(sa.text(
                    "INSERT INTO fu_users (email, hashed_password, is_active, is_superuser, is_verified) VALUES (:email, :hashed, :active, 0, 0)"
                ), {"email": email, "hashed": hashed, "active": int(is_active)})
            except Exception:
                # fallback: try dialect-agnostic upsert-like behaviour
                try:
                    conn.execute(sa.text(
                        "INSERT OR IGNORE INTO fu_users (email, hashed_password, is_active, is_superuser, is_verified) VALUES (:email, :hashed, :active, 0, 0)"
                    ), {"email": email, "hashed": hashed, "active": int(is_active)})
                except Exception:
                    # ignore failures for best-effort migration
                    pass
    finally:
        conn.close()
...
def downgrade():
    # No-op: do not drop user data on downgrade
    pass
