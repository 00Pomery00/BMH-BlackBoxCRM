...
def test_admin_webhook_dlq_and_requeue():
    ...
    r = client.get("/admin/webhooks", headers={"Authorization": f"Bearer {admin_token}"})
    assert r.status_code == 200
    data = r.json()
    assert any(wd["id"] == w.id for wd in data.get("webhooks", []))

    # requeue
    r2 = client.post(f"/admin/webhooks/{w.id}/requeue", headers={"Authorization": f"Bearer {admin_token}"})
