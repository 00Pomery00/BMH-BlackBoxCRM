...
def create_company(db: Session, name: str, description: str = "", lead_score: float = 0.0):
...
def create_lead(db: Session, lead: dict):
    ...
    return create_company(db, name=name, description=lead.get("description", ""), lead_score=score)
...
def create_or_update_lead(db: Session, lead: dict):
    ...
    if existing:
        ...
        if new_score > (existing.lead_score or 0.0):
            ...
            db.refresh(existing)
        return existing
    comp = create_company(db, name=name, description=lead.get("description", ""), lead_score=float(lead.get("lead_score", 0.0)))
    if email:
        contact = models.Contact(company_id=comp.id, name=lead.get("contact_name", name), email=email)
        db.add(contact)
        db.commit()
    return comp
