...
def register(payload: RegisterPayload):
    ...
    try:
        ...
        u = models.User(username=payload.username, email=payload.email, hashed_password=hashed)
        db.add(u)
        db.commit()
        db.refresh(u)
...
def login(payload: LoginPayload):
    ...
    try:
        ...
        if not security.pwd_context.verify(payload.password, u.hashed_password):
            raise HTTPException(status_code=401, detail="invalid credentials")
        token = security.create_access_token({"sub": u.username, "uid": u.id, "role": u.role})
...
def me(user: schemas.User = Depends(security.get_current_user)):
    return {"username": user.username, "role": user.role, "email": getattr(user, 'email', None)}
...
def list_users(user: schemas.User = Depends(security.get_current_user)):
    ...
    try:
        ...
        out = []
        for r in rows:
            out.append({"id": r.id, "username": r.username, "email": r.email, "role": r.role, "is_active": bool(r.is_active)})
        return {"users": out}
    finally:
        db.close()
...
@router.post("/role")
def change_role(payload: RoleChange, user: schemas.User = Depends(security.get_current_user)):
    security.require_role(user, ("admin",))
    db: Session = SessionLocal()
    try:
...
def deactivate_user(payload: DeactivatePayload, user: schemas.User = Depends(security.get_current_user)):
    security.require_role(user, ("admin",))
    db: Session = SessionLocal()
    try:
        target = db.query(models.User).filter(models.User.username == payload.username).first()
...
def change_password(payload: ChangePasswordPayload, user: schemas.User = Depends(security.get_current_user)):
    db: Session = SessionLocal()
    try:
