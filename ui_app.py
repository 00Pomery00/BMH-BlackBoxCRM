...
def run_streamlit():
    ...
    with col1:
        label_f1 = "IBS Activation Report (IBS_Activation_Report_BMH Saviors*" ".xlsx)"
        f1 = st.file_uploader(label_f1, type=["xlsx"], key="f1")
        f2 = st.file_uploader("Program.xlsx", type=["xlsx"], key="f2")
    with col2:
        label_f3 = "IBS Performance (IBS_Performance_BMH Saviors*" ".xlsx)"
    ...
    with st.spinner("Načítám soubory a počítám..."):
        ...
        strat_prod = pick_sheets(sheets2, strat_names) or pick_sheets(
            sheets3, ["Strat"]
        )
        ...
        def attach_coeffs(r: pd.Series) -> pd.Series:
            ...
            if c:
                ...
                return pd.Series(
                    [coef_main, coef_strat, bonus, total, c.get("note", "")]
                )
            else:
